...
public class LumberjackTradeOffers {
  ...
  public static class EmeraldsForOverworldLogs extends SingleItemTypeTrade {
    ...
    public TradeOffer create(Entity entity, Random random) {
      experience = 2;
      List<ItemConvertible> logs = new ArrayList<>(
          Arrays.asList(Blocks.ACACIA_LOG, Blocks.BIRCH_LOG, Blocks.DARK_OAK_LOG, Blocks.JUNGLE_LOG, Blocks.OAK_LOG,
              Blocks.SPRUCE_LOG, Blocks.STRIPPED_ACACIA_LOG, Blocks.STRIPPED_BIRCH_LOG, Blocks.STRIPPED_DARK_OAK_LOG,
  ...
  public static class FencesForEmeralds extends SingleItemTypeTrade {
    ...
    public TradeOffer create(Entity entity, Random random) {
      experience = 6;
      List<ItemConvertible> doors = random.nextFloat() < 0.5F ? new ArrayList<>(ItemTags.WOODEN_FENCES.values())
          : new ArrayList<>(BlockTags.FENCE_GATES.values());
      setInput(Items.EMERALD, 1);
  ...
  public static class DoorsForEmeralds extends SingleItemTypeTrade {
    ...
    public TradeOffer create(Entity entity, Random random) {
      experience = 10;
      List<Item> doors = random.nextFloat() < 0.5F ? ItemTags.WOODEN_DOORS.values()
          : ItemTags.WOODEN_TRAPDOORS.values();
      setInput(Items.EMERALD, 1);
      setOutput(doors.get(random.nextInt(doors.size())), 1);
      return super.create(entity, random);
    }
  ...
  public static class PlanksForLogs implements TradeOffers.Factory {
    ...
    public TradeOffer create(Entity entity, Random random) {
      ...
      ItemConvertible inp = logs.get(index);
      ItemConvertible out = planks.get(inp);

      return new TradeOffer(new ItemStack(inp, count), new ItemStack(Items.EMERALD, 1), new ItemStack(out, count * 6),
          5, 15, 0.05F);
    }
  }
  ...
  public static class BookshelfForEmeralds extends SingleItemTypeTrade {
    @Nullable
    @Override
    public TradeOffer create(Entity entity, Random random) {
      experience = 30;
      maxUses = random.nextInt(5) + 10;
      setInput(Items.EMERALD, 8);
      setOutput(Items.BOOKSHELF, 1);
      return super.create(entity, random);
    }
  }
  ...
  public static class WorkstationForEmeralds extends SingleItemTypeTrade {
    ...
    public TradeOffer create(Entity entity, Random random) {
      ...
      List<Block> workstations = new ArrayList<>(Arrays.asList(Blocks.FLETCHING_TABLE, Blocks.CRAFTING_TABLE,
          Blocks.CARTOGRAPHY_TABLE, Blocks.COMPOSTER, Blocks.LOOM));

      if (ModuleHandler.enabled("beekeeper"))
        workstations.add(CustomWorkstationHandler.getWorkstation(Beekeeper.class));
      workstations.add(CustomWorkstationHandler.getWorkstation(Lumberjack.class));
